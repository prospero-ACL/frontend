...
**Routing** (`src/config/create-router.tsx`): `react-router-dom` data router. Top-level split between logged-out (`/`, `HomePage`) and logged-in (`/app`, `MainApp` shell with nested routes for chat/profile/docs), gated by `ProtectedRouteIsLoggedIn` / `ProtectedRouteIsLoggedOut` in `src/config/route-protection.tsx`, which read `state.auth.isAuthenticated`/`isLoading` and redirect. Because auth state depends on the `onBootStrap` bootstrap thunk resolving, both guards render a loading state until `isLoading` is false.

**Feature modules** (`src/pages/main-app/modules/`): each module (e.g. `docs`) is self-contained with its own `components/`, `hooks/`, and an `index.tsx` entry — follow this layout for new modules rather than adding to shared component dirs.
